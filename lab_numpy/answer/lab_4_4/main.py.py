import numpy as np
from lab_4_4 import evaluation_test
...
input_str = [list(map(int,v[1:-2].replace(' ','').split(','))) for v in inputs]
input_pair = list()
for i in range(len(input_str)//2):
    input_pair.append([input_str[i*2], input_str[i*2+1]])

outputs = [evaluation_test(np.array(v[0]), np.array(v[1])) for v in input_pair]
